...
(function (global) {
  ...
  var BANDS = [
    { key: 'ji-ruo', lab: '極弱格', range: '<−60', name: '從弱' },
    { key: 'ruo', lab: '較弱格', range: '−60～−12', name: '扶抑身弱' },
    { key: 'wang', lab: '較旺格', range: '＋12～＋60', name: '扶抑身旺' },
    { key: 'ji-wang', lab: '極旺格', range: '>＋60', name: '專旺順洩' }
  ];
  var SPEC = {
  ...
  function specFor(dmWx, band, extraYong) {
    var key = SPEC[band] ? band : 'ruo';
    var pack = SPEC[key];
    var roles = rolesOf(dmWx);
    var yong = expand(roles, pack.yong);
    if (extraYong && yong.indexOf(extraYong) < 0) yong.unshift(extraYong);
    return {
      ruleVersion: 'mingju-l1-xiyong-v2',
      band: key,
      method: pack.name,
  ...
  function specFromMingJu(mj) {
    if (!mj) return null;
    return specFor(mj.dayMasterWx, mj.band, mj.tiaohou || '');
  }
  ...
  function matrixHTML(curWx, curBand) {
    ...
    return '<div class="yun-wrap xy-matrix"><div class="m6-block-title">第 1 層喜用規格</div>' +
      '<p class="yun-meta">只用淨分定帶。交界先調候為急，再按月令生扶→較旺／克洩耗→較弱。交界無獨立「印＋食傷」格。</p>' +
  ...
  function install() {
    ...
    E.l1XiyongFromMingJu = specFromMingJu;
