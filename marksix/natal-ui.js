(function(global){
  ...
  function render(){
    ...
    try{
      ...
      var spec=(mj&&E.l1XiyongFromMingJu)?E.l1XiyongFromMingJu(mj):(mj&&E.l1XiyongSpec)?E.l1XiyongSpec(mj.dayMasterWx, mj.band, mj.tiaohou):null;
      ...
      if(E.buildYun){
        try{
          var yunPack=E.buildYun(per.y,per.m,per.d,per.h,per.min,sex,new Date());
          yunHtml=yunHTML(yunPack, spec, yunTone);
          if(E.scoreYun && spec){
            var ys=E.scoreYun(mj, spec, yunPack);
            yunScore=(E.yunScoreHTML && ys)?E.yunScoreHTML(ys):'';
          }
        }catch(e){}
      }
      box.classList.remove('hidden');
