...
(function (global) {
  ...
  function hourZhiIndex(h) { return Math.floor((h + 1) / 2) % 12; }
  ...
  function dayGzAtNoon(y, m, d) {
    var solar = global.Solar.fromYmdHms(y, m, d, 12, 0, 0);
    var ec = solar.getLunar().getEightChar();
    if (ec && typeof ec.setSect === 'function') { try { ec.setSect(1); } catch (e) {} }
    return ec.getDay();
  }
  ...
  function pillarsFromClock(clk) {
    ...
    var note = '';
    if (clk.hour === 23) note = '24h 夜子23:00–23:59 · 日柱' + dayGz + '（當日）· 時柱' + hourGz + '（次日' + dunDay + '遁）';
    else if (clk.hour === 0) note = '24h 早子00:00–00:59 · 日柱' + dayGz + '（00:00已換日）· 時柱' + hourGz;
    return {
      year: ec.getYear(), month: ec.getMonth(), day: dayGz, hour: hourGz,
      dayMaster: dayGz.charAt(0), dayMasterWx: WX_G[dayGz.charAt(0)] || '',
      ziKind: clk.zi, clockNote: note
    };
  }
  function install() {
    ...
    E.timeGz = timeGz;
    if (!E.pillarsAt || !E.pillarsAt._zishiV3) {
      ...
      E.pillarsAt._zishiV3 = true; E.pillarsAt._zishi = true;
    }
    if (E.buildYun && !E.buildYun._zishiV3) {
      ...
      E.buildYun._zishiV3 = true; E.buildYun._zishi = true;
    }
    E._zishi = !!(E.pillarsAt && E.pillarsAt._zishiV3);
  }
  install();
  var n = 0, t = setInterval(function () {
    n++; install();
    if ((global.TXMarkSixEngine && global.TXMarkSixEngine.pillarsAt && global.TXMarkSixEngine.pillarsAt._zishiV3) || n > 40) clearInterval(t);
  }, 50);
