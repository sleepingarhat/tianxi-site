(function(){
  ...
  function renderPillarsChart(p,t){
    ...
    return '<div class="bz-wrap">'+'<div class="bz-title"><b>'+t+'</b> · 日主 <span class="wx-'+(WX_CLS_UI[det.dayMasterWx]||'')+'">'+det.dayMaster+'</span>（'+det.dayMasterWx+'）</div>'+'<div class="bz-chart">'+cols+'</div>'+'</div>';
  }
  function renderNextDraw(){
    ...
    var pillarsBlock='';
    if(NEXT.pillars){
      pillarsBlock='<div class="m6-next-pillars">'+'<div class="m6-next-k">八字四柱（攪珠 21:30）</div>'+renderPillarsChart(NEXT.pillars,'攪珠日')+'</div>';
    }
    el.innerHTML='<div class="m6-next-grid">'+'<div><div class="m6-next-k">下一期期數</div><div class="m6-next-v m6-next-draw">'+(NEXT.draw||'—')+'</div></div>'+'<div><div class="m6-next-k">攪珠日期時間</div><div class="m6-next-v">'+cnDate(NEXT.date)+' · '+NEXT.timeLabel+'</div></div>'+'</div>'+pillarsBlock;
  }
  function renderResult(){
    ...
    el.innerHTML='<div class="m6-rhead"><span class="draw">第 '+d.draw+' 期</span><span class="date">'+cnDate(d.date)+'</span>'+(d.snowball?'<span class="m6-snow">'+d.snowball+'</span>':'')+'</div>'+'<div class="m6-balls">'+ballsHTML+'</div>'+'<div class="m6-balls-cap">正碼六粒 · 特別號碼一粒</div>'+(prizeRows?'<table class="m6-prize"><thead><tr><th>獎級</th>'+(showUnits?'<th>中獎注數</th>':'')+(showDividend?'<th>每注派彩</th>':'')+'</tr></thead><tbody>'+prizeRows+'</tbody></table>':'')+'<div class="m6-rfoot">'+'<div class="m6-stat"><div class="k">投注額</div><div class="v">'+money(d.totalInvestment)+'</div></div>'+'<div class="m6-stat"><div class="k">頭獎基金</div><div class="v">'+money(d.jackpot||d.firstPrizeDividend)+'</div></div>'+'</div>';
  ...
  function renderStats(){
    ...
    var html='';order.forEach(function(n){var heat=maxCount?(s.count[n]/maxCount)*100:0;
      html+='<div class="m6-cell"><div class="heat" style="height:'+heat+'%"></div>'+ball(n,'sm')+'<div class="row"><span>開</span><b>'+s.count[n]+'</b></div><div class="row"><span>遺</span><b class="'+(s.curOm[n]>=10?'due':'')+'">'+s.curOm[n]+'</b></div><div class="row"><span>期</span><b>'+s.maxOm[n]+'</b></div></div>';});
    grid.innerHTML=html;
  }
  ...
  function panHTML(pan,t){
    ...
    return '<div class="m6-meta"><b>'+t+'</b></div>';
  }
  ...
  function runBaziCombo(){
    if(!engineReady('personalBazi')||!NEXT){setStatus('引擎未載入完整版，請強制重新整理頁面',true);return;}
    var E=engine(); var per=readPersonal();
  ...
  function runQimenCombo(){
    if(!engineReady('personalQimen')||!NEXT){setStatus('引擎未載入完整版，請強制重新整理頁面',true);return;}
    var E=engine(); var per=readPersonal();
    if(!per){setStatus('請先輸入個人出生資料（含時辰）',true);return;}
    showPanel('qimen');
  ...
  function runBacktest(kind){
    ...
    if(needFn&&typeof E[needFn]!=='function'){setStatus('引擎未載入完整版，請強制重新整理頁面',true);return;}
    ...
    function step(){
      for(var k=0;k<6&&i<slice.length;k++,i++){
        ...
        tr.innerHTML='<td class="col-meta">'+meta+'</td>'+'<td><div class="m6-nums-mini">'+pred.numbers.map(function(x){return ballHit(x,sc.hit_zheng,sp);}).join('')+'</div></td>'+'<td>'+officialBalls(nums,sp,sc)+'</td>'+'<td>'+hitBalls(sc,sp)+'</td>'+'<td class="col-prize m6-score'+(pl.hi?' hi':'')+'">'+pl.txt+'</td>';
        tbody.appendChild(tr);
      }
      if(i<slice.length){ setStatus('回測 '+i+'/'+slice.length+'…'); setTimeout(step,0); }
      else {
        document.getElementById('btStats').innerHTML='<div class="m6-statgrid">'+'<div class="m6-statbox"><div class="k">整體平均命中</div><div class="v">'+(sum/n).toFixed(3)+'</div></div>'+'<div class="m6-statbox"><div class="k">≥5 字期數</div><div class="v">'+eq5+'</div></div>'+'<div class="m6-statbox"><div class="k">最高單期</div><div class="v">'+maxS+'</div></div>'+'<div class="m6-statbox"><div class="k">回測期數</div><div class="v">'+n+'</div></div></div>';
  ...
  function boot(){
    ...
    Promise.all([
      ...
      if(HISTORY.length){
        ...
        if(engine()&&engine().pillarsAt){ try{NEXT.pillars=engine().pillarsAt(NEXT.y,NEXT.m,NEXT.d,21);}catch(e){} }
        renderNextDraw();
      }
