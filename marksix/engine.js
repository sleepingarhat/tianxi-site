...
(function (global) {
  ...
  var YANG_JU = {
    ...
    驚蟾: [1, 7, 4], 惊蛰: [1, 7, 4], 春分: [3, 9, 6], 清明: [4, 1, 7], 穀雨: [5, 2, 8], 谷雨: [5, 2, 8],
  ...
  function buildZhuanPan(di, yang, origin, target, zhiShiPalace) {
    var homeStar = {}, homeDoor = {}, homeGan = {};
    for (var i = 0; i < 8; i++) {
      var p = RING[i];
    ...
    var stars = applyRot(homeStar, origin, target);
    var gans = applyRot(homeGan, origin, target);
    var doors = applyRot(homeDoor, origin, zhiShiPalace);
    stars[5] = '天禽';
    gans[5] = di[5];
    doors[5] = '';
    var ruiPalace = 2;
    Object.keys(stars).forEach(function (pk) { if (stars[+pk] === '天芮') ruiPalace = +pk; });
    if (ruiPalace !== 5) stars[ruiPalace] = '芮禽';
    var gods = {}, godList = yang ? GODS_YANG : GODS_YIN;
    var ti = ringIdx(target);
    if (ti < 0) ti = 0;
  ...
  function castQimen(y, m, d, hour) {
    ...
    Object.keys(di).forEach(function (pk) {
      var p = +pk;
      if (di[p] === targetGan && p !== 5) { zf = p; sgp = p; }
    });
    var zs = flyZhiShi(origin, xun.ganPos, yj.yang);
    var palaces = buildZhuanPan(di, yj.yang, origin, zf, zs);
    return {
  ...
  global.TXMarkSixEngine = {
    ...
    ruleVersion: 'bazi + qimen-chaibu-v3 + zhuanpan'
  };
